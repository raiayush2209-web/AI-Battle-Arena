          ...
          <div className="bg-gray-900/50 border border-gray-800 rounded-2xl p-6">
            ...
            <div className="flex justify-end mt-3">
              <button
                ...
                className="px-6 py-2.5 bg-gradient-to-r from-purple-600 to-cyan-600 hover:from-purple-500 hover:to-cyan-500 disabled:from-gray-700 disabled:to-gray-700 disabled:cursor-not-allowed text-white font-medium rounded-xl transition-all duration-200 flex items-center gap-2"
              >
                {loading ? (
                  <>
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            <div className="relative">
              ...
              <div className="absolute inset-0 w-16 h-16 border-4 border-transparent rounded-full animate-spin border-b-cyan-500" style={{ animationDirection: "reverse", animationDuration: "1.5s" }}></div>
            </div>
        ...
        {result && !loading && (
          <div className="space-y-8 animate-in">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Solution 1 - Mistral */}
              <div className={`bg-gray-900/50 border rounded-2xl overflow-hidden ${
                ...
                <div className="px-5 py-3 border-b border-gray-800/50 flex items-center justify-between bg-gray-900/30">
                  <div className="flex items-center gap-2">
                    ...
                    {(result.judge?.winner?.includes("Solution 1") || result.judge?.winner?.includes("Mistral")) && (
                      <span className="text-xs bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded-full">🏆 Winner</span>
                    )}
                  </div>
                  <div className={`text-2xl font-bold ${getScoreColor(result.judge?.solution_1_score)} ring-2 ${getScoreRing(result.judge?.solution_1_score)} rounded-full w-10 h-10 flex items-center justify-center text-sm`}>
              ...
              <div className={`bg-gray-900/50 border rounded-2xl overflow-hidden ${
                ...
                <div className="px-5 py-3 border-b border-gray-800/50 flex items-center justify-between bg-gray-900/30">
                  ...
                  <div className={`text-2xl font-bold ${getScoreColor(result.judge?.solution_2_score)} ring-2 ${getScoreRing(result.judge?.solution_2_score)} rounded-full w-10 h-10 flex items-center justify-center text-sm`}>
                ...
                <div className="flex items-center justify-center gap-8">
                  <div className="text-center">
                    ...
                    <p className={`text-3xl font-bold ${getScoreColor(result.judge?.solution_1_score)}`}>
                      {result.judge?.solution_1_score}<span className="text-sm text-gray-500">/10</span>
                    </p>
                  </div>
                  ...
                  <div className="text-center">
                    ...
                    <p className={`text-3xl font-bold ${getScoreColor(result.judge?.solution_2_score)}`}>
                      {result.judge?.solution_2_score}<span className="text-sm text-gray-500">/10</span>
                    </p>
                  </div>
                </div>
